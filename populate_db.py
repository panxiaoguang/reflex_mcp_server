...
def get_category_from_path(file_path: str) -> str:
    ...
    if 'library' in path_parts:
        ...
        if library_idx + 1 < len(path_parts):
            return path_parts[library_idx + 1].replace('_', ' ').replace('-', ' ').title()
    return "Other"
                ...
                for file in files:
                    if file.endswith('.md'):
                        ...
                        try:
                            ...
                            name, description = extract_component_info(content, file_path)
                            category = get_category_from_path(file_path)
                            
                            # Check if component already exists
                            existing = session.exec(select(Component).where(Component.name == name)).first()
                            if existing:
                                name = f"{name} ({category})"  # Make name unique
                            ...
                            print(f"Added component: {name} ({category})")
            ...
            for file in files:
                if file.endswith('.md'):
                    ...
                    try:
                        ...
                        existing = session.exec(select(DocSection).where(DocSection.name == name)).first()
                        if existing:
                            name = f"{name} ({section})"  # Make name unique
                        ...
                        print(f"Added doc: {name} ({section})")
